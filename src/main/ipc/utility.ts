...
export function registerUtilityHandlers(ipcMain: IpcMain): void {
  ...
  ipcMain.handle('read-file-by-path', handleReadFileByPath);
  ipcMain.handle('write-file-by-path', handleWriteFileByPath);
  ipcMain.handle('get-home-dir', handleGetHomeDir);
  ipcMain.handle('get-managed-claude-path', handleGetManagedClaudePath);
...
export function removeUtilityHandlers(ipcMain: IpcMain): void {
  ...
  ipcMain.removeHandler('read-file-by-path');
  ipcMain.removeHandler('write-file-by-path');
  ipcMain.removeHandler('get-home-dir');
  ipcMain.removeHandler('get-managed-claude-path');
...
/**
 * Handler for 'read-file-by-path' IPC call.
 * Reads a file by absolute path with safety checks.
 */
async function handleReadFileByPath(
  _event: IpcMainInvokeEvent,
  absolutePath: string
...
/**
 * Handler for 'write-file-by-path' IPC call.
 * Writes a file by absolute path with safety checks.
 */
async function handleWriteFileByPath(
  _event: IpcMainInvokeEvent,
  absolutePath: string,
  content: string
): Promise<{ success: boolean; error?: string }> {
...
async function handleGetHomeDir(): Promise<string> {
  return homedir();
}

/**
 * Handler for 'get-managed-claude-path' IPC call.
 * Returns the path to the system-managed CLAUDE.md (from the Claude Code installation).
 */
async function handleGetManagedClaudePath(): Promise<string | null> {
  ...
  if (platform === 'win32') {
    ...
    candidates.push('/Library/Application Support/ClaudeCode/CLAUDE.md');
  } else {
    candidates.push('/opt/claude-code/CLAUDE.md');
    candidates.push('/usr/share/claude-code/CLAUDE.md');
  }
