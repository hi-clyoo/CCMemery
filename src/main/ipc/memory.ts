 ...
 * - memory:saveFile                → { success, error? }
 * - memory:deleteFile              → { success, error? }
 * - memory:createFile              → { success, error?, file? }
...
const MEMORY_CREATE_FILE = 'memory:createFile';
...
export function registerMemoryHandlers(ipcMain: IpcMain): void {
  ...
  ipcMain.handle(MEMORY_CREATE_FILE, handleCreateFile);
...
export function removeMemoryHandlers(ipcMain: IpcMain): void {
  ...
  ipcMain.removeHandler(MEMORY_CREATE_FILE);
...
async function handleSaveFile(
  ...
  try {
    await registry.getActive().memoryReader.writeFile(projectIdResult.value, fileName, content);
    return { success: true };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    logger.error('Error in memory:saveFile:', error);
    return { success: false, error: message };
  }
}
...
async function handleDeleteFile(
  ...
  try {
    await registry.getActive().memoryReader.deleteFile(projectIdResult.value, fileName);
    return { success: true };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    logger.error('Error in memory:deleteFile:', error);
    return { success: false, error: message };
  }
}
...
async function handleCreateFile(
  ...
  try {
    ...
    const typeStr = typeof type === 'string' ? type : 'project';
    const nameStr = typeof name === 'string' ? name : finalName.replace(/\.md$/, '');
    const descStr = typeof description === 'string' ? description : '';
    const content = `---\nname: ${nameStr}\ndescription: ${descStr}\ntype: ${typeStr}\n---\n\n`;
    await registry.getActive().memoryReader.writeFile(projectIdResult.value, finalName, content);
    return { success: true, fileName: finalName, content };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    logger.error('Error in memory:createFile:', error);
    return { success: false, error: message };
  }
}
