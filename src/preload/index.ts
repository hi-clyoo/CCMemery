...
const electronAPI: ElectronAPI = {
  ...
  // Read/write files by absolute path
  readFileByPath: (absolutePath: string) =>
    ipcRenderer.invoke('read-file-by-path', absolutePath) as Promise<{ success: boolean; content?: string; error?: string }>,
  writeFileByPath: (absolutePath: string, content: string) =>
    ipcRenderer.invoke('write-file-by-path', absolutePath, content) as Promise<{ success: boolean; error?: string }>,
  getHomeDir: () => ipcRenderer.invoke('get-home-dir') as Promise<string>,
  getManagedClaudePath: () => ipcRenderer.invoke('get-managed-claude-path') as Promise<string | null>,
  ...
  memory: {
    ...
    saveFile: (projectId: string, fileName: string, content: string): Promise<MemoryOpenResult> =>
      ipcRenderer.invoke(MEMORY_SAVE_FILE, projectId, fileName, content) as Promise<MemoryOpenResult>,
    deleteFile: (projectId: string, fileName: string): Promise<MemoryOpenResult> =>
      ipcRenderer.invoke(MEMORY_DELETE_FILE, projectId, fileName) as Promise<MemoryOpenResult>,
    createFile: (
      projectId: string,
      fileName: string,
      type: string,
      ...
      ipcRenderer.invoke(MEMORY_CREATE_FILE, projectId, fileName, type, name, description) as Promise<CreateFileResult>,
